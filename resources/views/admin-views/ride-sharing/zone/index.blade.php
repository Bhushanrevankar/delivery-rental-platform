...
@section('title',translate('messages.ride_sharing_zone'))
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>
                    {{translate('messages.add_new_ride_sharing_zone')}}
                </span>
            </h1>
        </div>
        <!-- End Page Header -->
        <div class="row g-3">
            <div class="col-12">
                <form action="{{route('admin.ride.zone.store')}}" method="post" id="zone_form" class="shadow--card">
                    @csrf
                    ...
                    <div class="table-responsive datatable-custom">
                        <table id="columnSearchDatatable"
                            ...
                            <tr>
                                ...
                                <th class="border-0" >{{translate('messages.riders')}}</th>
                            ...
                            @foreach($zones as $key=>$zone)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{route('admin.ride.zone.status',[$zone['id'],$zone->status?0:1])}}" method="get" id="status-{{$zone['id']}}">
                                        </form>
                                    </td>
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a class="btn action-btn btn--primary btn-outline-primary"
                                                href="{{route('admin.ride.zone.edit',['zone'=>$zone['id']])}}" title="{{translate('messages.edit')}} {{translate('messages.zone')}}"><i class="tio-edit"></i>
                                            </a>
                                            ...
                                            <form action="{{route('admin.ride.zone.destroy',['zone'=>$zone['id']])}}" method="post" id="zone-{{$zone['id']}}">
                                                @csrf @method('delete')
                                            </form>
                                        </div>
                                    </td>
...
@push('script_2')
    ...
    <script>
        ...
        {
            $.get({
                url: "{{route('admin.ride.zone.coordinates')}}",
                dataType: 'json',
    ...
    <script>
        $('#zone_form').on('submit', function (e) {
            ...
            $.post({
                url: "{{route('admin.ride.zone.store')}}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                ...
                success: function (data) {
                    ...
                    if (data.errors) {
                        ...
                        toastr.success("{{translate('messages.ride_sharing_zone_added_successfully')}}", {
                            CloseButton: true,
                            ProgressBar: true
                        });
                        setTimeout(function () {
                            location.href = "{{route('admin.ride.zone.index')}}";
                        }, 2000);
                    }
                }
            });
