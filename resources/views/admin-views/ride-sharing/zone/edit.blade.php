...
@section('title',translate('messages.edit_ride_sharing_zone'))
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>
                    {{translate('messages.edit_ride_sharing_zone')}}
                </span>
            </h1>
        </div>
        <!-- End Page Header -->
        <form action="{{route('admin.ride.zone.update',['zone'=>$zone->id] )}}" method="post" id="zone_form" class="shadow--card">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-5">
...
<script>
    ...
    {
        $.get({
            url: '{{route('admin.ride.zone.coordinates')}}/{{$zone->id}}',
            dataType: 'json',
...
<script>
    $('#zone_form').on('submit', function (e) {
        ...
        $.post({
            url: '{{route('admin.ride.zone.update',['zone'=>$zone->id])}}',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            ...
            success: function (data) {
                ...
                if (data.errors) {
                    ...
                    toastr.success("{{translate('messages.ride_sharing_zone_updated_successfully')}}", {
                        CloseButton: true,
                        ProgressBar: true
                    });
                    setTimeout(function () {
