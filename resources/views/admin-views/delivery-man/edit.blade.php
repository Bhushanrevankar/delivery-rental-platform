                    ...
                    <div class="row g-4">
                        <div class="col-lg-8">
                            ...
                            data-original-title="{{ translate('messages.Required.')}}"> *
                                    ...
                                    <select name="zone_ids[]" class="form-control  js-select2-custom" multiple required>
                                    @foreach(\App\Models\Zone::all() as $zone)
                                        @if(isset(auth('admin')->user()->zone_id))
                                            @if(auth('admin')->user()->zone_id == $zone->id)
                                                <option value="{{$zone->id}}" {{in_array($zone->id, $deliveryMan->zones->pluck('id')->toArray())?'selected':''}}>{{$zone->name}}</option>
                                            @endif
                                        @else
                                        <option value="{{$zone->id}}" {{in_array($zone->id, $deliveryMan->zones->pluck('id')->toArray())?'selected':''}}>{{$zone->name}}</option>
                                        @endif
