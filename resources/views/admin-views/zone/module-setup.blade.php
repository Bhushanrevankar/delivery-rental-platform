                        ...
                        <select name="module_id[]" id="choice_modules" class="form-control js-select2-custom"
                            ...
                            @foreach ($modules as $module)
                                <option value="{{ $module['id'] }}"
                                    ...
                                    {{ $module['module_name'] }}{{ !$module['status'] ? ' (Disabled)' : '' }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="delivery_charge_options" id="delivery_charge_options">
                        ...
                        @if (count($zone->modules) > 0)
                            @foreach ($zone->modules as $module)
                                ...
                                @elseif ($module->module_type == 'ride_sharing')
                                    <div class="row gy-1 module-row" id="module_{{ $module->id }}">
                                        <div class="col-sm-4"><input type="text" class="form-control"
                                                value="{{ $module->module_name }}"
                                                placeholder="{{ translate('messages.choice_title') }}" readonly></div>
                                        <div class="col-sm-2"><input type="number" class="form-control"
                                                name="module_data[{{ $module->id }}][per_km_shipping_charge]"
                                                step=".01" min="0"
                                                placeholder="{{ translate('Set charge from ride category') }}"
                                                data-toggle="tooltip" data-placement="right"
                                                data-original-title="{{ translate('messages.You have to set category wise charge from ride category') }}"
                                                readonly></div>
                                        <div class="col-sm-2"><input type="number" step=".01" min="0"
                                                class="form-control"
                                                name="module_data[{{ $module->id }}][minimum_shipping_charge]"
                                ...
                                @elseif ($module->module_type == 'rental' && addon_published_status('Rental'))
                                    <div class="row gy-1 module-row" id="module_{{ $module->id }}">
                                        ...
                                        <div class="col-sm-2"><input type="number" class="form-control"
                                                name="module_data[{{ $module->id }}][per_km_shipping_charge]"
                                                step=".01" min="0"
                                                placeholder="{{ translate('Not applicable') }}" data-toggle="tooltip"
                                                data-placement="right"
                                                data-original-title="{{ translate('messages.Not applicable') }}" readonly>
                                        </div>
                                        <div class="col-sm-2"><input type="number" step=".01" min="0"
                                                class="form-control"
                                                name="module_data[{{ $module->id }}][minimum_shipping_charge]"
...
@push('script_2')
    ...
    <script>
        ...
        $('#choice_modules').on('change', function() {
            ...
            $.each($("#choice_modules option:selected"), function(index, element) {
                ...
                if (ids.includes($(this).val())) {
                    ...
                    } else if (found.module_type == 'ride_sharing') {
                        add_ride_sharing_module($(this).val(), name.trim());
                    } else {
        ...
        function add_ride_sharing_module(i, name) {
            let n = name;
            $('#delivery_charge_options').append(
                '<div class="row gy-1 module-row" id="module_' + i +
                '"><div class="col-sm-4"><input type="text" class="form-control" value="' + n +
                '" placeholder="{{ translate('messages.choice_title') }}" readonly></div><div class="col-sm-2"><input type="number" name="module_data[' +
                i +
                '][per_km_shipping_charge]" class="form-control" step=".01" min="0" placeholder="{{ translate('Set charge from ride category') }}" value="" title="{{ translate('messages.per_km_delivery_charge') }}" readonly></div><div class="col-sm-2"><input type="number" name="module_data[' +
                i +
                '][minimum_shipping_charge]" step=".01" min="0" class="form-control" placeholder="{{ translate('Set charge from ride category') }}" value="" title="{{ translate('messages.Minimum delivery charge') }}" readonly></div><div class="col-sm-2"><input type="number" name="module_data[' +
                i +
                '][maximum_shipping_charge]" step=".01" min="0" class="form-control" placeholder="{{ translate('Set charge from ride category') }}" value="" title="{{ translate('messages.maximum delivery charge') }}" readonly></div><div class="col-sm-2"><input type="number" step=".01" min="0" class="form-control" name="module_data[' +
                i +
