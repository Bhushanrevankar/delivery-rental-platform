...
Route::group(['namespace' => 'Admin', 'as' => 'admin.'], function () {
    ...
    Route::group(['middleware' => ['admin', 'current-module']], function () {
        ...
        //Ride sharing
        Route::group(['namespace' => 'Ride', 'prefix' => 'ride-sharing', 'as' => 'ride.', 'middleware' => ['module:customerList']], function () {
            Route::get('zone/cordinates/{id?}', 'RideZoneController@get_zone_cordinates')->name('zone.coordinates');
            Route::get('status/{id}/{status}', 'RideZoneController@status')->name('zone.status');
            Route::resource('zone', 'RideZoneController');
            Route::get('vehicle-category/update-status/{vehicle_category}/{status}', 'RideCategoryController@update_status')->name('vehicle-category.status');
            Route::resource('vehicle-category', 'RideCategoryController');
            Route::get('requests', 'App\Http\Controllers\Admin\Ride\RideRequestController@index')->name('request.index');
            Route::get('requests/{id}', 'App\Http\Controllers\Admin\Ride\RideRequestController@show')->name('request.show');
            Route::get('settings', 'RideSharingController@index')->name('settings');
            Route::post('settings', 'RideSharingController@update');
            Route::get('report', 'RideSharingController@report')->name('report');
        });
    });
});
