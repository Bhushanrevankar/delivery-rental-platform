...
Route::group(['namespace' => 'Api\V1', 'middleware' => 'localization'], function () {
    Route::put('/test-put-method', function () {
        return response()->json(['data' => 'sample']);
    })->withoutMiddleware('localization');
    ...
    Route::group(['prefix' => 'delivery-man'], function () {
        ...
        Route::group(['middleware' => ['dm.api']], function () {
            ...
            Route::get('subscription/cards', 'DeliverymanController@get_used_prepaid_cards');
            Route::post('subscription/reload', 'DeliverymanController@reload');
            ...
            Route::group(['prefix' => 'ride'], function () {
                Route::get('new-request', 'DeliverymanController@new_ride_requests');
                Route::get('request', 'DeliverymanController@ride_index');
                Route::get('request/{ride_request_id}', 'DeliverymanController@show_ride_request');
                Route::put('request', 'DeliverymanController@update_ride_request');
            });
        });
    });
    ...
    Route::group(['middleware' => ['module-check']], function () {
        ...
        Route::group(['prefix' => 'customer', 'middleware' => 'auth:api'], function () {
            ...
            //Ride sharing
            Route::group(['prefix' => 'ride'], function () {
                Route::get('request', [RideRequestController::class, 'index']);
                Route::post('request', [RideRequestController::class, 'store']);
                Route::get('request/{ride_request_id}', [RideRequestController::class, 'show']);
                Route::put('request/{ride_request_id}', [RideRequestController::class, 'update']);
                Route::post('request/review', [RideRequestController::class, 'submit_review']);
                Route::post('request/calculate-credits', [RideRequestController::class, 'calculate_credits'])->withoutMiddleware('auth:api');
            });
        });
        ...
        Route::group(['prefix' => 'customer', 'middleware' => 'apiGuestCheck'], function () {
            Route::group(['prefix' => 'order'], function () {
                ...
                Route::post('calculate-credits', 'OrderController@calculate_credits');
            ...
            //Ride sharing
            Route::get('ride/category', [RideRequestController::class, 'ride_category']);
        });
