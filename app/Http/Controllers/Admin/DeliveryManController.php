...
class DeliveryManController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $ride_zone_id = $request->query('ride_zone_id', 'all');

        $delivery_men = DeliveryMan::when(is_numeric($zone_id), function ($query) use ($zone_id) {
            return $query->whereHas('zones', function ($q) use ($zone_id) {
                $q->where('zones.id', $zone_id);
            });
        })->when(is_numeric($ride_zone_id), function ($query) use ($ride_zone_id) {
            return $query->whereHas('ride_zones', function ($q) use ($ride_zone_id) {
                $q->where('ride_zones.id', $ride_zone_id);
            });
        })
            ->with(['zones', 'ride_zones'])
            ->where('type', 'zone_wise')
            ->latest()
            ->paginate(config('default_pagination'));

        $zone = is_numeric($zone_id) ? Zone::findOrFail($zone_id) : null;
        $ride_zone = is_numeric($ride_zone_id) ? RideZone::findOrFail($ride_zone_id) : null;

        return view('admin-views.delivery-man.list', compact('delivery_men', 'zone', 'ride_zone'));
    }
    ...
    {
        $dm = DeliveryMan::with(['reviews', 'zones', 'ride_zones', 'vehicle_category'])->where('type', 'zone_wise')->where(['id' => $id])->first();
        if ($tab == 'info') {
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'zone_ids.required' => translate('messages.select_a_zone'),
            'zone_ids.min' => translate('messages.select_a_zone'),
        ...
        $dm->delivery = (bool)($request->deliveryman_type == 'delivery' || $request->deliveryman_type == 'both');
        $dm->ride_sharing = (bool)($request->deliveryman_type == 'ride_sharing' || $request->deliveryman_type == 'both');
        $dm->vehicle_reg_no = $request->vehicle_reg_no;
        $dm->vehicle_rc = $request->file('vehicle_rc') ? Helpers::upload('delivery-man/rc/', 'png', $request->file('vehicle_rc')) : null;;
        $dm->vehicle_owner_noc = $request->file('vehicle_owner_noc') ? Helpers::upload('delivery-man/noc/', 'png', $request->file('vehicle_owner_noc')) : null;
        $dm->ride_category_id = $request->ride_category_id;
        $dm->save();

        $dm->zones()->attach($request->input('zone_ids', []));
        $dm->ride_zones()->attach($request->input('ride_zone_ids', []));
    ...
    public function update(Request $request, $id)
    {
        ...
        $delivery_man->delivery = (bool)($request->deliveryman_type == 'delivery' || $request->deliveryman_type == 'both');
        $delivery_man->ride_sharing = (bool)($request->deliveryman_type == 'ride_sharing' || $request->deliveryman_type == 'both');
        $delivery_man->vehicle_reg_no = $request->vehicle_reg_no;
        $delivery_man->vehicle_rc = $request->file('vehicle_rc') ? Helpers::update('delivery-man/rc/', $delivery_man->vehicle_rc, 'png', $request->file('vehicle_rc')) : $delivery_man->vehicle_rc;
        $delivery_man->vehicle_owner_noc = $request->file('vehicle_owner_noc') ? Helpers::update('delivery-man/noc/', $delivery_man->vehicle_owner_noc, 'png', $request->file('vehicle_owner_noc')) : $delivery_man->vehicle_owner_noc;
        ...
        $delivery_man->zones()->sync($request->input('zone_ids', []));
        $delivery_man->ride_zones()->sync($request->input('ride_zone_ids', []));
