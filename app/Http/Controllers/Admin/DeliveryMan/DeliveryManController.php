...
class DeliveryManController extends BaseController
{
    ...
    public function __construct(
        ...
        protected RideZoneRepositoryInterface $rideZoneRepo,
    ...
    private function getListView(Request $request): View
    {
        ...
        $rideZoneId = $request->query('ride_zone_id', 'all');
        ...
        $deliveryMen = $this->deliveryManRepo->getFilterWiseListWhere(
            ...
            relations: ['zones', 'ride_zones', 'wallet'],
            dataLimit: config('default_pagination')
        );

        $zone = is_numeric($zoneId) ? $this->zoneRepo->getFirstWhere(params: ['id' => $zoneId]) : null;
        $rideZone = is_numeric($rideZoneId) ? $this->rideZoneRepo->getFirstWhere(params: ['id' => $rideZoneId]) : null;

        return view(DeliveryManViewPath::LIST[VIEW], compact('deliveryMen', 'zone', 'rideZone'));
    }
    ...
    public function add(DeliveryManAddRequest $request): Application|Redirector|RedirectResponse
    {
        ...
        $deliveryMan->zones()->attach($request->zone_ids);
    ...
    {
        $deliveryMan = $this->deliveryManRepo->getFirstWithoutGlobalScopeWhere(params: ['id' => $id], relations: ['zones']);
        $language = getWebConfig('language');
        $defaultLang = str_replace('_', '-', app()->getLocale());
    ...
    public function update(DeliveryManUpdateRequest $request, $id): Application|Redirector|RedirectResponse
    {
        ...
        $deliveryMan->zones()->sync($request->zone_ids);
