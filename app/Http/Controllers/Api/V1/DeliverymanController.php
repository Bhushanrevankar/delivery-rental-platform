...
class DeliverymanController extends Controller
{
    public function get_profile(Request $request)
    {
        ...
        $dm['subscription_remaining_days'] = (int) $dm->subscription_days;
    ...
    public function get_latest_orders(Request $request)
    {
        ...
        $zoneIds = $dm->zones()->select('zones.id')->pluck('id');

        $orders = Order::with(['customer', 'store', 'parcel_category']);

        // NOTE: This is where the current order listings send to delivery-man app
        // we have two types of DMs, zone_wise, and store bound.
        if ($dm->type == 'zone_wise') {
            $orders = $orders->whereIn('zone_id', $zoneIds)
            ...
            // TODO: Parked idea of notify deliveryman in radius 20 km.
            // $last_location = $dm->last_location;

            // if ($last_location != null) {
            //     $start_lat = $last_location->latitude;
            //     $start_long = $last_location->longitude;

            //     $orders = $orders->select(
            //         'orders.*',
            //         DB::raw("(6371 * acos(cos(radians($start_lat)) * cos(radians(orders.pickup_lat)) * cos(radians(orders.pickup_long) - radians($start_long)) + sin(radians($start_lat)) * sin(radians(orders.pickup_lat)))) AS distance")
            //     )->havingRaw("distance <= 20"); // NOTE: 20 KM only
            // }
        } else {
        ...
        $orders = $orders->dmOrder()
            ...
            ->where('driver_credits_required', '<=', $dm->credit_balance) // Only show orders driver can afford
            ->orderBy('schedule_at', 'desc')
            ->get();
        $orders = Helpers::order_data_formatting($orders, true);
        return response()->json($orders, 200);
    ...
    public function accept_order(Request $request)
    {
        ...
        // TODO: remove this cash on hand check later.
        ...
        // Check and deduct driver credits
        if ($order->driver_credits_required > 0) {
            if ($dm->credit_balance < $order->driver_credits_required) {
                return response()->json([
                    'errors' => [
                        ['code' => 'insufficient_credits', 'message' => translate('messages.insufficient_credit_balance_to_accept_order')]
                    ]
                ], 403);
            }

            // Deduct driver credits using pessimistic locking
            ...
            $walletService->deductCredits(
                ...
                details: 'Credits deducted for accepting order #' . $order->id
            );

            $order->driver_credits_status = 'deducted';
        }
    ...
    public function update_order_status(Request $request)
    {
        ...
        if ($request->status == 'delivered') {
            ...
            // Refund credits to customer, driver, and merchant if applicable
            $order->order_status = 'canceled'; // Set status before refund so the service can check eligibility
            $order->save();
            $creditRefundService = app(\App\Services\CreditRefundService::class);
            $creditRefundService->refundOrderCredits($order);
    ...
    //Ride sharing
    public function new_ride_requests(Request $request)
    {
        ...
        $query = RideRequest::with(['customer', 'zone', 'vehicle_category'])
            ->whereIn('zone_id', $zoneIds)
            // ->where('ride_category_id', $dm->ride_category_id)
            ->whereHas('vehicle_category', function ($q) use ($dm) {
                $q->where('dm_vehicle_id', $dm->vehicle_id);
            })
            ->where('ride_status', 'pending')
            ->where(function ($q) use ($dm) {
                $q->whereNull('delivery_man_id')->orWhere('delivery_man_id', $dm->id);
            })
            ->where('driver_credits_required', '<=', $dm->credit_balance); // Only show rides driver can afford

        $last_location = $dm->last_location;
        // NOTE: This is anti pattern to what I want to implement. Because if deliveryman doesn't have last location,
        // it will show ride requests that are not in the radius.
        if ($last_location != null) {
            $query->whereDistance('pickup_point', new Point($last_location->latitude, $last_location->longitude), '<=', 20000); // NOTE: search radius 20 km.
        }

        $ride_requests = $query->latest()
            ->paginate($limit, '*', 'ride_request', $offset);
    ...
    public function ride_index(Request $request)
    {
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();
        $limit = $request->query('limit', 10);
        $offset = $request->query('offset', 1);
        $status = $request->query('status', null);
        $ride_requests = RideRequest::with(['customer', 'zone', 'vehicle_category'])->where('delivery_man_id', $dm->id)
            ->when($status, function ($query) use ($status) {
                if ($status == 'on_going') {
                    $query->whereIn('ride_status', ['accepted', 'picked_up', 'arrived']);
                } else {
                    $query->where('ride_status', $status);
                }
            })
            ->latest()->paginate($limit, '*', 'ride_request', $offset);
        return response()->json([
            'limit' => $limit,
            'offset' => $offset,
            'total_size' => $ride_requests->total(),
    ...
    public function update_ride_request(Request $request, RideSharingLogics $rideSharingLogics)
    {
        $validator = Validator::make($request->all(), [
            'ride_request_id' => 'required',
            'lat' => 'required_if:status,picked_up,completed,canceled|numeric|nullable',
            'lng' => 'required_if:status,picked_up,completed,canceled|numeric|nullable',
            'distance' => 'required_if:status,completed,canceled|numeric|nullable',
            'otp' => 'required_if:status,picked_up',
            'status' => 'required|in:accepted,ignored,picked_up,completed,canceled,arrived,reached',
        ]);
        ...
        if (!$dm
            // || !$dm->ride_sharing
        ) return response()->json(['errors' => [['code' => 'status', 'message' => translate('messages.sorry_ride_sharing_disable_for_you')]]], 403);

        $zoneIds = $dm->zones()->select('zones.id')->pluck('id');

        $ride_request = RideRequest::whereIn('zone_id', $zoneIds)->find($request->ride_request_id);

        if ($request['status'] == 'picked_up' && $ride_request->otp != $request['otp']) {
            return response()->json([
                'errors' => [
                    ['code' => 'otp', 'message' => translate('messages.otp_not_matched')]
                ]
            ], 406);
        }

        if (!$ride_request) return response()->json(['errors' => [['code' => 'status', 'message' => translate('messages.ride_request_not_found')]]], 403);

        if ($request->status == 'ignored') {
            $rideSharingLogics->create_ignore_log($request->ride_request_id, $dm->id, $ride_request->ride_zone_id, $ride_request->ride_category_id);
            return response()->json(['message' => translate('messages.ride_request_ignored_successfully')], 200);
        }

        if ($request->status == 'accepted' && $ride_request->ride_status != 'pending') return response()->json(['errors' => [['code' => 'status', 'message' => translate('messages.already_accepted_or_moved')]]], 403);

        if (in_array($request->status, ['picked_up', 'completed', 'canceled'])  && $ride_request->delivery_man_id != $dm->id) return response()->json(['errors' => [['code' => 'status', 'message' => translate('messages.sorry_this_request_is_not_accepted_by_you')]]], 403);

        if ($request->status == 'picked_up' && $ride_request->delivery_man_id != $dm->id) return response()->json(['errors' => [['code' => 'status', 'message' => translate('messages.sorry_this_request_is_not_accepted_by_you')]]], 403);

        if ($request->status == 'picked_up') {
            $ride_request->actual_pickup_point = new Point($request->lat, $request->lng);
            $ride_request->pickup_time = now();
        } elseif (in_array($request->status, ['completed', 'canceled'])) {
            if ($request->status == 'canceled') {
                // For canceled rides, just set the points and time, no fare calculation
                if ($request->lat && $request->lng) {
                    $ride_request->actual_dropoff_point = new Point($request->lat, $request->lng);
                }
                $ride_request->dropoff_time = now();
                if ($request->distance) {
                    $ride_request->actual_distance = $request->distance;
                }
            } else {
                // For completed rides, calculate fare
                $ride_request->actual_dropoff_point = new Point($request->lat, $request->lng);
                $ride_request->dropoff_time = now();
                $ride_request->actual_distance = $request->distance;
                $ride_request->actual_time = now()->diffInMinutes(Carbon::parse($ride_request->pickup_time));
                $ride_request->actual_fare = $rideSharingLogics->calculate_fare($ride_request->vehicle_category, $request->distance, $ride_request->actual_time, $ride_request->estimated_time);
                $ride_request->tax = $ride_request->actual_fare * $ride_request->tax / 100;
                $ride_request->total_fare = $ride_request->tax + $ride_request->actual_fare;
            }
        } elseif ($request->status == 'accepted') {
            $ride_request->delivery_man_id = $dm->id;

            // Check and deduct driver credits when accepting ride request
            if ($ride_request->driver_credits_required > 0) {
                if ($dm->credit_balance < $ride_request->driver_credits_required) {
                    return response()->json([
                        'errors' => [
                            ['code' => 'insufficient_credits', 'message' => translate('messages.insufficient_credit_balance_to_accept_ride')]
                        ]
                    ], 403);
                }

                // Deduct driver credits
                $walletService = app(\App\Services\WalletService::class);
                $walletService->deductCredits(
                    user: $dm,
                    amount: $ride_request->driver_credits_required,
                    transaction_type: 'usage',
                    reference: 'ride:' . $ride_request->id,
                    details: 'Credits deducted for accepting ride request #' . $ride_request->id
                );

                $ride_request->driver_credits_status = 'deducted';
            }
        }
        ...
        // Refund credits when driver cancels the ride
        if ($request->status === 'canceled') {
            $creditRefundService = app(\App\Services\CreditRefundService::class);
            $creditRefundService->refundRideCredits($ride_request);
        }

        return response()->json(['message' => translate('messages.ride_request_status_updated_successfully')], 200);
    }

    public function show_ride_request(Request $request, $ride_request_id)
    {
        if (!$ride_requests = RideRequest::with(['customer', 'rider', 'zone', 'vehicle_category'])->find($ride_request_id)) return response()->json(['errors' => [['code' => 'ride_request_id', 'message' => translate('messages.ride_request_not_found')]]], 403);

        return response()->json(new RideRequestResource($ride_requests), 200);
    }

    public function get_used_prepaid_cards(Request $request)
    {
        $dm = DeliveryMan::with(['rating'])->where(['auth_token' => $request['token']])->first();
        return response()->json(['prepaid_cards' => $dm->used_prepaid_cards], 200);
    }
    ...
    public function reload(Request $request)
    {
        $dm = DeliveryMan::with(['rating'])->where(['auth_token' => $request['token']])->first();
        $validator = Validator::make($request->all(), [
            'card_no' => 'required|exists:prepaid_cards,card_no',
            'pin' => 'required',
        ], [
            'card_no.required' => 'Card number is required.',
            'card_no.exists' => 'Card number does not exists.',
        ...
        $prepaidCard = PrepaidCard::where('card_no', $card_no)->where('is_used', false)->first();

        if ($prepaidCard == null) {
            return response()->json(['errors' => Helpers::error_formater('card-not-found', 'Card not found.')], 403);
        }

        if ($prepaidCard->pin != $pin) {
            return response()->json(['errors' => Helpers::error_formater('invalid-pin', 'Invalid PIN.')], 403);
        }

        DB::beginTransaction();

        $dm->usePrepaidCard($prepaidCard);
        $dm->save();

        $prepaidCard->is_used = true;
        $prepaidCard->usable()->associate($dm);
        $prepaidCard->save();

        DB::commit();

        return response()->json(['message' => 'Subscription reloaded.'], 200);
    }
