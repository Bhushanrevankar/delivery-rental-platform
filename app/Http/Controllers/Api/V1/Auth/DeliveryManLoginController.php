...
class DeliveryManLoginController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (auth('delivery_men')->attempt($data)) {
            ...
            if ($delivery_man->zones->isNotEmpty()) {
                // TODO: Reimplement this to support multiple zones, currently only one zone is supported for now.
                $first_zone = $delivery_man->zones->first();
                $zone_topic =  $delivery_man->type == 'zone_wise' ? $first_zone->deliveryman_wise_topic . '_push' : '';

                if ($delivery_man->vehicle_id) {
                    $topic = 'delivery_man_' . $first_zone->id . '_' . $delivery_man->vehicle_id;
                } else {
                    $topic = $delivery_man->type == 'zone_wise' ? $first_zone->deliveryman_wise_topic : 'restaurant_dm_' . $delivery_man->store_id;
                }
            }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'zones.required' => translate('messages.select_a_zone'),
