...
class OrderController extends Controller
{
    ...
    // TODO: duplicated logic in PlaceNewOrder trait 
    /**
     * Calculate credit cost for an order
     * 
    ...
    public function calculate_credits(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'user_type' => 'nullable|in:customer,driver,merchant',
        ]);
        ...
        try {
            ...
            $orderAmount = $request->input('price');
            ...
            if ($module != null && $module->module_type == 'ride_sharing') {
                ...
                $rule = \Modules\Subscription\Entities\CreditDeductionRule::where('user_type', $userType)
            ...
            info('Ride hailing rule', ['is_found' => $rule != null]);
            ...
            info('Price range rule', ['is_found' => $rule != null]);
            ...
            info('Distance range rule', ['is_found' => $rule != null]);

            // Calculate credits based on rule or fallback logic
            $creditsRequired = 0;
            $ruleName = null;
            $ruleCondition = null;
