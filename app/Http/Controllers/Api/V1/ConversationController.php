    ...
    public function messages(Request $request)
    {
        ...
        if (isset($conversation)) {
            if ($conversation->sender_type == 'vendor' && $conversation->sender) {
                ...
                if (1 > $order) {
                    $order = RideRequest::where('user_id', $user->user_id)->where('delivery_man_id', $user2->id)->whereIn('ride_status', ['accepted', 'picked_up', 'arrived'])->count();
                }
    ...
    public function dm_messages(Request $request)
    {
        ...
        if ($conversation) {
            if ($conversation->sender_type == 'vendor' && $conversation->sender) {
                ...
                $order = Order::where('delivery_man_id', $dm->id)->where('user_id', $user->id)->whereIn('order_status', ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
                if (1 > $order) {
                    $order = RideRequest::where('user_id', $user->id)->where('delivery_man_id', $dm->id)->whereIn('ride_status', ['accepted', 'picked_up', 'arrived'])->count();
                }
