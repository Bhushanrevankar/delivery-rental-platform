...
class Helpers
{
        ...
        $postData = [
            'message' => [
                ...
                "android" => [
                    "priority" => "high",
                    "notification" => [
                        // TODO: Should be dynamic set in dashboard.
                        "channel_id" => 'BBBring',
                    ]
                ],
                "apns" => [
    ...
    // public static function send_push_notif_to_devices(array $fcm_tokens, array $data, ?string $web_push_link = null)
    // {
    //     $conversation_id = $data['conversation_id'] ?? '';
    //     $sender_type = $data['sender_type'] ?? '';
    //     $module_id = $data['module_id'] ?? '';
    ...
    //     $batch_endpoint = "https://fcm.googleapis.com/batch";

    //     try {
    //         $access_token = self::getAccessToken($key);
    //         $boundary = 'my_custom_boundary_' . uniqid();

    //         $body = '';
    //         $post_path = '/v1/projects/' . $project_id . '/messages:send';

    //         foreach ($fcm_tokens as $index => $token) {
    //             $payload = json_encode(
    //                 [
    //                     'message' => [
    //                         "token" => $token,
    ...
    //             // Pass the full content type with the boundary directly to withBody()
    ...
    //         return false;
    //     } catch (\Exception $e) {
    //         info('Fcm response error', ['error' => $e->getMessage()]);
    //         return false;
    //     }
    // }
    ...
    // NOTE: search radius 20 km.
    public static function query_nearest_deliveryman($zone_id, $start_lat, $start_long, $vehicle_id = null, $ride_category_id = null, $distance = 20, $limit = 10, $credits_required = 0)
    {
        $latest_locations = DB::table('delivery_histories')
            ->select('delivery_man_id', 'latitude', 'longitude')
        ...
        $query = DeliveryMan::active()
            ->joinSub($latest_locations, 'last_location', function ($join) {
                $join->on('delivery_men.id', '=', 'last_location.delivery_man_id');
            })
            ->whereNotNull('fcm_token')
            ->whereNotNull('last_location.latitude')
            ->whereNotNull('last_location.longitude')
            ->select(
                'delivery_men.*',
                DB::raw("(6371 * acos(cos(radians($start_lat)) * cos(radians(last_location.latitude)) * cos(radians(last_location.longitude) - radians($start_long)) + sin(radians($start_lat)) * sin(radians(last_location.latitude)))) AS distance"),
            )
            ->whereHas('zones', function ($q) use ($zone_id) {
                $q->where('zones.id', $zone_id);
            });

        if ($ride_category_id != null) {
            $query->where('ride_category_id', $ride_category_id);
        }
        ...
        // Filter by credit balance using the creditWallet relationship
        if ($credits_required > 0) {
            $query->whereHas('creditWallet', function ($q) use ($credits_required) {
                $q->where('balance', '>=', $credits_required);
            });
        }
    ...
    // TODO: This function is used to send order notification to the customer.
    // Implement notification for delivery man by device id and proximity.
    // TODO: check flow of calls of this function. log in laravel.log
        ...
        try {
            ...
            if ($order->order_type == 'delivery' && !$order->scheduled && $status == 'pending' && $order->payment_method == 'cash_on_delivery' && config('order_confirmation_model') == 'deliveryman') {
                if ($order->store->sub_self_delivery && $push_notification_status) {
                    ...
                    if ($order->zone && self::getNotificationStatusData('deliveryman', 'deliveryman_order_notification', 'push_notification_status')) {
                        ...
                        $fcm_tokens = self::query_nearest_deliveryman(
                            zone_id: $order->zone_id,
                            start_lat: $delivery_address['latitude'],
                            start_long: $delivery_address['longitude'],
                            vehicle_id: $order->dm_vehicle_id,
                            credits_required: $order->driver_credits_required,
                        )->pluck('fcm_token')->toArray();
                        Log::info('fcm_tokens queried for order request - delivery', ['fcm_tokens' => $fcm_tokens]);
            ...
            if ($order->order_type == 'parcel' && in_array($order->order_status, ['pending', 'confirmed'])) {
                ...
                if ($order->zone && self::getNotificationStatusData('deliveryman', 'deliveryman_order_notification', 'push_notification_status')) {
                    ...
                    Log::info('fcm_tokens queried for order request - parcel', ['fcm_tokens' => $fcm_tokens]);
                    foreach ($fcm_tokens as $fcm_token) {
                        self::send_push_notif_to_device($fcm_token, $data);
                    }
                }
            ...
            if ($order->order_type == 'delivery' && !$order->scheduled && $order->order_status == 'confirmed'  && ($order->payment_method != 'cash_on_delivery' || config('order_confirmation_model') == 'store')) {
                ...
                if ($order->store->sub_self_delivery && $push_notification_status) {
                    ...
                    if ($order->zone && self::getNotificationStatusData('deliveryman', 'deliveryman_order_notification', 'push_notification_status')) {
                        ...
                        Log::info('fcm_tokens queried for order request - delivery confirmed vendor', ['fcm_tokens' => $fcm_tokens]);
                        foreach ($fcm_tokens as $fcm_token) {
                            self::send_push_notif_to_device($fcm_token, $data);
                        }
                    }
