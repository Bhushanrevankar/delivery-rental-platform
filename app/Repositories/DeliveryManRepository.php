...
class DeliveryManRepository implements DeliveryManRepositoryInterface
{
    ...
    public function getFilterWiseListWhere(string $zoneId = 'all', string $rideZoneId = 'all', string $searchValue = null, array $filters = [],  string $additionalFilter = null,  string $jobType = null, array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $key = explode(' ', $searchValue);
        $data = $this->deliveryMan->with($relations)->where($filters)
            ->when(is_numeric($zoneId), function ($query) use ($zoneId) {
            ...
            ->when(is_numeric($rideZoneId), function ($query) use ($rideZoneId) {
                return $query->whereHas('ride_zones', function ($q) use ($rideZoneId) {
                    $q->where('ride_zones.id', $rideZoneId);
                });
            })
