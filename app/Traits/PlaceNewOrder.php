...
trait PlaceNewOrder
{
    ...
    public function new_place_order(Request $request, $prescription = false)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'order_attachment' => $prescription ? ['required'] : 'nullable',
        ]);
        ...
        try {
            ...
            $order->prescription_order = $prescription ? 1 : 0;
            $additionalCharges = [];
            ...
            if ($request->order_type !== 'parcel') {
                ...
                if (!$prescription  && $store->minimum_order > $product_price + $total_addon_price) {
                    DB::rollBack();
                    return response()->json([
                        'errors' => [
            ...
            // Calculate credits for all user types
            $creditRequirements = $this->calculateAllUserTypeCredits($request);

            $order->customer_credits_required = $creditRequirements['customer_credits_required'];
            $order->driver_credits_required = $creditRequirements['driver_credits_required'];
            $order->merchant_credits_required = $creditRequirements['merchant_credits_required'];

            // Check if customer has enough credits (only for non-guest users)
            if ($request->user && $creditRequirements['customer_credits_required'] > 0) {
                $customer = $request->user;

                if ($customer->credit_balance < $creditRequirements['customer_credits_required']) {
                    DB::rollBack();
                    return response()->json([
                        'errors' => [
                            ['code' => 'insufficient_credits', 'message' => translate('messages.insufficient_credit_balance_to_place_order')]
                        ]
                    ], 403);
                }

                // Deduct customer credits immediately (before order is saved, but will commit with DB transaction)
                $walletService = app(WalletService::class);
                $walletService->deductCredits(
                    user: $customer,
                    amount: $creditRequirements['customer_credits_required'],
                    transaction_type: 'usage',
                    reference: 'order:pending:' . ($order->id ?? 'new'),
                    details: 'Credits deducted for order placement'
                );

                $order->customer_credits_status = 'deducted';
            } else {
                $order->customer_credits_status = 'none';
            }

            // Check and deduct merchant credits (for non-parcel orders)
            if ($request['order_type'] !== 'parcel' && isset($store) && $creditRequirements['merchant_credits_required'] > 0) {
                ...
                if (!$merchant) {
                    ...
                    return response()->json([
                        'errors' => [
                            ['code' => 'merchant_not_found', 'message' => translate('messages.merchant_not_found')]
                        ]
                    ], 403);
                }

                if ($merchant->credit_balance < $creditRequirements['merchant_credits_required']) {
                    DB::rollBack();
                    return response()->json([
                        'errors' => [
                            ['code' => 'merchant_insufficient_credits', 'message' => translate('messages.merchant_has_insufficient_credits')]
                        ]
                    ], 403);
                }

                // Deduct merchant credits immediately
                $walletService = app(WalletService::class);
                $walletService->deductCredits(
                    user: $merchant,
                    amount: $creditRequirements['merchant_credits_required'],
                    transaction_type: 'usage',
                    reference: 'order:pending:' . ($order->id ?? 'new'),
                    details: 'Credits deducted for receiving order'
                );

                $order->merchant_credits_status = 'deducted';
            } else {
                $order->merchant_credits_status = 'none';
            }

            // Set initial credit status for driver (will be deducted when driver accepts)
            $order->driver_credits_status = 'none';
    ...
    // TODO: Wild logic here. remove this later.
    private function calculate_order_credits(float $order_amount): int
    {
        // Fallback to old logic if no rules are found
        if ($order_amount < 11) {
            return 4; // 3 for small order fee + 1 credit
        } elseif ($order_amount <= 30) {
            return 1;
        } elseif ($order_amount <= 50) {
            return 2;
        } elseif ($order_amount <= 100) {
            return 3;
        }
    ...
    // TODO: duplicate logic in calculate_credits in OrderController.php
    /**
     * Calculate credits required for an order based on dynamic rules
     */
    private function calculateDynamicOrderCredits(Request $request, $user): float
    {
        $userType = 'customer'; // Default to customer for now
        ...
        $rule = CreditDeductionRule::where('user_type', $userType)
            ...
            ->where(function ($query) use ($orderAmount) {
                $query->where('min_value', '<=', $orderAmount)
                    ->where(function ($q) use ($orderAmount) {
                        $q->where('max_value', '>=', $orderAmount)
                            ->orWhereNull('max_value');
                    });
            })
            ->first();

        // If no price range rule found, try distance range for delivery orders
        if (!$rule && $request['order_type'] === 'delivery' && $distance > 0) {
            $rule = CreditDeductionRule::where('user_type', $userType)
        ...
        // Return the credits to deduct or fallback to old calculation
        return $rule ? $rule->credits_to_deduct : $this->calculate_order_credits($orderAmount);
    }

    /**
     * Calculate credits for a specific user type based on dynamic rules
     *
     * @param string $userType - 'customer', 'driver', or 'merchant'
     * @param float $orderAmount - Total order amount
     * @param float $distance - Distance in km (for delivery orders)
     * @param string $orderType - 'delivery', 'take_away', or 'parcel'
     * @param int|null $moduleId - Module ID for module-specific rules
     * @return float
     */
    private function calculateCreditsForUserType(
        string $userType,
        float $orderAmount,
        float $distance = 0,
        ?int $moduleId = null
    ...
    /**
     * Calculate credits required for all user types (customer, driver, merchant)
     *
     * @param Request $request
     * @return array
     */
    private function calculateAllUserTypeCredits(Request $request): array
    {
        $orderAmount = $request['order_amount'] ?? 0;
        $distance = $request['distance'] ?? 0;
        ...
        return [
            'customer_credits_required' => $this->calculateCreditsForUserType(
                userType: 'customer',
                orderAmount: $orderAmount,
                distance: $distance,
                moduleId: $moduleId
            ),
            'driver_credits_required' => $this->calculateCreditsForUserType(
                userType: 'driver',
                orderAmount: $orderAmount,
                distance: $distance,
                moduleId: $moduleId
            ),
            'merchant_credits_required' => $this->calculateCreditsForUserType(
                userType: 'merchant',
                orderAmount: $orderAmount,
                distance: $distance,
                moduleId: $moduleId
